...
b3Vector3 btSoftbodyTriangleCL::GetPointByBaryCoord(const btSoftbodyCL* pCloth, float a, float b, float c) const
{
	...
	return b3Vector3(v0.m_Pos*a + v1.m_Pos*b + v2.m_Pos*c);
}

b3Vector3 btSoftbodyTriangleCL::GetVelocityByBaryCoord(const btSoftbodyCL* pCloth, float a, float b, float c) const
{
	const btSoftbodyNodeCL& v0 = pCloth->getVertexArray()[m_IndexVrx[0]];
	const btSoftbodyNodeCL& v1 = pCloth->getVertexArray()[m_IndexVrx[1]];
	const btSoftbodyNodeCL& v2 = pCloth->getVertexArray()[m_IndexVrx[2]];

	return b3Vector3(v0.m_Vel*a + v1.m_Vel*b + v2.m_Vel*c);
}
